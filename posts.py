...
def get_posts(channel_name):
    if channel_name == "":
        sql = "SELECT P.content, U.username, P.sent_at, C.name, P.id FROM Posts P, Users U, Channels C WHERE P.user_id=U.id AND C.id=P.channel_id AND P.visible=1 ORDER BY P.id DESC"
        result = db.session.execute(sql)
    else:
        sql = "SELECT P.content, U.username, P.sent_at, C.name, P.id FROM Posts P, Users U, Channels C WHERE P.user_id = U.id AND C.name=LOWER(:channel_name) AND C.id=P.channel_id AND P.visible=1 ORDER BY P.id DESC"
...
def get_post(post_id):
    sql = "SELECT P.content, U.username FROM Posts P, Users U WHERE P.id=:post_id AND U.id=P.user_id AND P.visible=1"
